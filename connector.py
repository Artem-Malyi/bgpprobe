...
def handlePackets(p):
	...
	if p[TCP].flags == 18 and not handshakeOk: 
		...
		ack = IP(dst=p[IP].src, id=int(RandShort())) / TCP(sport=p[TCP].dport, dport=p[TCP].sport, ack=p[TCP].seq+1, seq=p[TCP].ack, flags="A")
	...
	if handshakeOk and not bgpOpenSent:
		bgpOpen = IP(dst=p[IP].src, id=int(RandShort())) / TCP(sport=p[TCP].dport, dport=p[TCP].sport, ack=p[TCP].seq+1, seq=p[TCP].ack, flags="PA") / BGPHeader(type=1) / BGPOpen(version=4, AS=65002, hold_time=180, bgp_id=myIp)
	...
	if bgpOpenSent and p.haslayer(BGPOpen) and not firstKeepAliveSent:
		...
		ack2 = IP(dst=p[IP].src, id=int(RandShort())) / TCP(sport=p[TCP].dport, dport=p[TCP].sport, ack=p[TCP].seq+p[BGPHeader].len+pl[BGPHeader].len, seq=p[TCP].ack, flags="A")
		print "[i] sending ACK packet:", ack2.summary(), "len:", p[BGPHeader].len
		send (ack2)
		keepAlive = IP(dst=p[IP].src, id=int(RandShort())) / TCP(sport=p[TCP].dport, dport=p[TCP].sport, ack=p[TCP].seq+p[BGPHeader].len+pl[BGPHeader].len, seq=p[TCP].ack, flags="PA") / BGPHeader(type=4, len=19)
	...
	if p.haslayer(BGPHeader) and p[BGPHeader].type == 4:
		...
		ack2 = IP(dst=p[IP].src, id=int(RandShort())) / TCP(sport=p[TCP].dport, dport=p[TCP].sport, ack=p[TCP].seq+p[BGPHeader].len, seq=p[TCP].ack, flags="A")
		print "[i] sending ACK packet:", ack2.summary()
		send (ack2)
		#if not firstKeepAlive:
		#	keepAlive = IP(dst=p[IP].src, id=int(RandShort())) / TCP(sport=p[TCP].dport, dport=p[TCP].sport, ack=p[TCP].seq+p[BGPHeader].len, seq=p[TCP].ack, flags="PA") / BGPHeader(type=4, len=19)
	...
	if p.haslayer(BGPHeader) and p[BGPHeader].type == 2:
		...
		#updatePacket = IP(dst=p[IP].src, id=int(RandShort())) / TCP(sport=p[TCP].dport, dport=p[TCP].sport, ack=p[TCP].seq+p[BGPHeader].len, seq=p[TCP].ack, flags="PA") / Raw(load=update)
		send(IP(dst=p[IP].src, id=int(RandShort())) / TCP(sport=p[TCP].dport, dport=p[TCP].sport, ack=p[TCP].seq+p[BGPHeader].len, seq=p[TCP].ack, flags="PA")/
		               BGPHeader(type=2) / BGPUpdate(nlri=(24, '192.168.2.0'),
...
def bgpConnect(ipAddr):
	...
	syn = IP(dst=ipAddr, id=int(RandShort())) / TCP(sport=srcPort, dport=179, ack=0, seq=randomSeq, flags="S")
